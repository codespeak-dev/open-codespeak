...
class EnsureServerStarts(Phase):
    ...
    def __init__(self):
        self.logger = logging.getLogger(EnsureServerStarts.__class__.__qualname__)
    ...
    def run_server(self, project_path: str) -> [bool, str]:
        ...
        self.logger.info(f"Launching server: {args} in folder {project_path} and waiting for up to {timeout_sec} seconds")
        proc = None
        try:
            ...
            while True:
                ...
                try:
                    ...
                    if response.status_code in accepted_status_codes:
                        ...
                        self.logger.info(f"Request to {url} failed with status code {response.status_code}, response: {response.content}")

                except RequestException as e:
                    self.logger.info(f"Request to {url} failed with exception {e}")
                ...
                if sel.get_map():
                    ...
                    for key, _ in events:
                        ...
                        if not data:
                            # EOF
                            self.logger.info(f"Unregistering {'STDOUT' if key.fileobj == proc.stdout else 'STDERR'}")
                            sel.unregister(key.fileobj)
                        else:
                            output_line = data.rstrip('\n')
                            if key.fileobj == proc.stdout:
                                self.logger.info(f"Received STDOUT line: \"{output_line}\" after {elapsed:.2f} s since start")
                                output_lines.append(output_line)
                            else:
                                self.logger.info(f"Received STDERR line: \"{output_line}\" after {elapsed:.2f} s since start")
                                output_lines.append(output_line)
    ...
    def run(self, state: State, context: Context) -> dict:
        ...
        for i in range(attempts):
            try:
                ...
                if success:
                    self.logger.info(f"Server started successfully on attempt {i + 1}")
                    break
                else:
                    self.logger.info(f"Attempt {i + 1}: Server failed to start. Output:[[[\n{output}\n]]]")
                    self.try_to_fix_server(project_path, output, context)
            except Exception as e:
                self.logger.info(f"Attempt {i + 1}: Exception occurred: {e}")
        else:
